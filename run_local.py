"""
Local runner - PostgreSQL backend.
...
os.environ.setdefault(
    'DATABASE_URL',
    'postgresql://asset_admin:asset_secure_pass_2026@localhost:5432/asset_management'
)
...
def add_column_if_missing(engine, table_name, column_name, column_type='VARCHAR(200)'):
...
with setup_app.app_context():
    ...
    # Add new columns to tickets table if missing (for existing DBs)
    add_column_if_missing(db.engine, 'tickets', 'priority', 'VARCHAR(20) DEFAULT \'medium\'')
    add_column_if_missing(db.engine, 'tickets', 'assigned_to', 'VARCHAR(200)')
    add_column_if_missing(db.engine, 'tickets', 'category', 'VARCHAR(100)')
...
print("  DB    : PostgreSQL (asset_management)")
